...
import com.tongji.software_management.redis.EnumRedisKey;
import com.tongji.software_management.websocket.ChatWebsocket;
...
public class MatchCacheUtil {
     ...
     */
    public String getUserInMatchRandom(String userId,String contestId) {
        Optional<Map.Entry<Object, Object>> any = redisTemplate.opsForHash().entries(EnumRedisKey.USER_STATUS.getKey())
                .entrySet().stream().filter(entry -> entry.getValue().equals(StatusEnum.IN_MATCH.getValue()) && !entry.getKey().equals(userId) && redisTemplate.opsForHash().get(EnumRedisKey.USER_CONTEST_INFO.getKey(),entry.getKey()).equals(contestId)).findAny();
        return any.map(entry -> entry.getKey().toString()).orElse(null);
    }

    public String getUserInMatchRandom(String userId,String contestId,String exceptId) {
        Optional<Map.Entry<Object, Object>> any = redisTemplate.opsForHash().entries(EnumRedisKey.USER_STATUS.getKey())
                .entrySet().stream().filter(entry -> entry.getValue().equals(StatusEnum.IN_MATCH.getValue()) && !entry.getKey().equals(userId)&&!entry.getKey().equals(exceptId) && redisTemplate.opsForHash().get(EnumRedisKey.USER_CONTEST_INFO.getKey(),entry.getKey()).equals(contestId)).findAny();
        return any.map(entry -> entry.getKey().toString()).orElse(null);
    }
    ...
    public void setUserInRoom(String userId1, String userId2,String userId3) {
        redisTemplate.opsForHash().put(EnumRedisKey.ROOM.getKey(), userId1, userId2+'#'+userId3);
        redisTemplate.opsForHash().put(EnumRedisKey.ROOM.getKey(), userId2, userId1+'#'+userId3);
        redisTemplate.opsForHash().put(EnumRedisKey.ROOM.getKey(), userId3, userId1+'#'+userId2);
    }
     ...
     * 移除用户的加入对战id
     */
    public void removeUserContestInfo(String userId) {
        redisTemplate.opsForHash().delete(EnumRedisKey.USER_CONTEST_INFO.getKey(), userId);
    }

    /**
     * 设置用户的加入对战id
     */
    public void setUserContestInfo(String userId, String contestId) {
        removeUserContestInfo(userId);
        redisTemplate.opsForHash().put(EnumRedisKey.USER_CONTEST_INFO.getKey(), userId, contestId);
    }
